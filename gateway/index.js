...
async function periodicTest(deltaSeconds) {
  console.log(
    `[TEST MONITOR] Running proxy health checks every ${deltaSeconds}s`,
  );
  setInterval(async () => {
...
(async () => {
  ...
  if (!success) {
    console.log(
      `[GATEWAY STOPPED] One or more proxies failed initial test. Fix them and retry.`,
    );
    process.exit(1);
  }
