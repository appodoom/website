...
export function page1script(pageId) {
  ...
  function readProbabilityMatrixFromEditor() {
    ...
    const matrix = Array.from(
      { length: probabilityRowLabels.length },
      () => [],
    );
  ...
  function validateProbabilityMatrix(matrix) {
    ...
    for (let column = 0; column < maxSubd; column += 1) {
      const total = matrix.slice(1).reduce((sum, row) => sum + row[column], 0);
      if (total > 100.0001) {
