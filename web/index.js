...
app.post("/web/api/register/", async (req, res) => {
  ...
  try {
    ...
    if (created) {
      ...
      return res.redirect("/web/wait-room/");
    } else {
      throw new Error("");
    }
  } catch (e) {
    res.status(400).json({
      error:
        e && e.message && e.message == "Validation error"
...
app.use("/web/shared/", express.static(SHARED_FILES));
...
app.use("/web/wait-room/", findRole, express.static(WAIT_ROOM));
app.use("/web/", express.static(LANDING_PAGE));
